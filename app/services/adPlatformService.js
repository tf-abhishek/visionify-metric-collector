...
exports.downloadAndSaveAdPlatformAssets = async function (adPlatformData, forceDownload = true) {
  ...
  for (const campaign of adPlatformData) {
    ...
    // In the case of a coolerTakeover, the adType must be grabbed at the asset level instead of at the campaign level
    if (campaign.AdType === 'CoolerTakeover') {
      await saveCoolerTakeoverAdAssets(campaign, forceDownload);
    } else {
      await saveAdAssets(campaign, forceDownload);
    }
  }
};


const saveAdAssets = async (campaign, forceDownload) => {
  ...
  if (!dirToSaveTo) {
    logger.error(`Error: Unfamiliar AdType encountered in ${campaign.AdType} campaign ${campaign.CampaignId}: ${campaign.AdType}. Skipping.`);
    return;
  }
  
  for (let assetFilename in assetsToSave) {
...
const saveCoolerTakeoverAdAssets = async (campaign, forceDownload) => {
  const assetsToSave = utils.toDistinctDictionary(campaign.Assets.filter(asset => asset.SasLink && asset.FileName),
    asset => asset.FileName, asset => asset);
  for (let assetFilename in assetsToSave) {
    if (assetsToSave.hasOwnProperty(assetFilename)) {
      const asset = assetsToSave[assetFilename];
      const dirToSaveTo = _assetTypeToLocalDirectory[asset.AdType];
      if (!dirToSaveTo) {
        logger.error(`Error: Unfamiliar AdType encountered in ${campaign.AdType} campaign ${campaign.CampaignId}: ${campaign.AdType}. Skipping.`);
        return;
      }
